#!/usr/bin/env node
// OpenAI smoke test for drift-ci. Runs init -> run -> baseline init -> run
// against the real OpenAI API.
//
// Usage:
//   OPENAI_API_KEY=sk-... node scripts/openai-smoke.mjs
//   OPENAI_API_KEY=sk-... OPENAI_MODEL=gpt-4o-mini node scripts/openai-smoke.mjs
//
...
if (!API_KEY) {
  console.error('drift-ci: OPENAI_API_KEY not set — skipping OpenAI smoke.');
  console.error('Set OPENAI_API_KEY to run this smoke test against the real API.');
  process.exit(0);
}
...
const workdir = mkdtempSync(join(tmpdir(), 'drift-openai-smoke-'));
console.log(`drift-ci: openai smoke workdir = ${workdir}`);
console.log(`drift-ci: model=${MODEL}`);

try {
  const init = run('drift-ci init', ['init', '--provider', 'openai', '--model', MODEL], workdir);
...
`;
  ...
  console.log(`\n=== OpenAI smoke green — total ${total} ms across 4 commands ===`);
} finally {
